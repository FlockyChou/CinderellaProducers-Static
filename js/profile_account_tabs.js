...
// 		$.get('/ajax/ownedcards/?back_to_profile&ids=' + ownedcards_to_reload.join(',') + '&page_size=' + ownedcards_to_reload.length, function(data) {
// 		    var html = $(data);
...
function loadCards(pane, account) {
	...
	if (data.trim() == '') {
	    ...
	    pane.html('<br><a href="' + window.location.pathname + '?account' + account_id + '=Cards#' + account_id + '" class="btn-card" data-toggle="tooltip" title="' + gettext('Permalink') + '"><i class="flaticon-link"></i></a>' + data);
	    loadToolTips();
	    ajaxModals();
	}
    });
